...
class Mat(object):
    ...
    def __matmul__(self, b):
        '''
         Wrapper so that 'v @ b' is the same as 'v.mul(b)'
        '''
        return mul(self, b)

    # def __rmatmul__(self, a):
    #     '''
    #      Wrapper so that 'x @ b' is the same as 'b.mul(b)'
    #     '''
    #     return mul(a, self)
...
def logistic(a):
    '''
     ...
     Applies the logistic function to each element.
    '''
    return Logistic([a])()
...
# Loss functions ========
def squared_error(a, target):
    '''
     v = squared_error(a, target)

     Computes the squared error for each sample (to each row).
     If the inputs are PxN, then v is Px1.
    '''
    return SquaredError([a], target)()

def cross_entropy(a, target):
    '''
     v = cross_entropy(a, target)

     Computes the cross-entropy for each sample (to each row).
     If the inputs are PxN, then v is Px1.
    '''
    return CrossEntropy([a], target)()
...
class Logistic(MatOperation):
    def __call__(self):
        v = Mat(1./( 1. + np.exp(-self.args[0].val) ))
        self.deriv = v.val * (1.-v.val)
        v.creator = self
        return v
...
class SquaredError(MatOperation):
    '''
    ...
    def __call__(self):
        self.deriv = self.args[0].val - self.target
        v = Mat(0.5 * np.sum(self.deriv**2, axis=1))
        v.creator = self
        return v
...
class CrossEntropy(MatOperation):
    '''
    ...
    def __call__(self):
        ...
        aval = self.args[0].val  # returns a numpy array
        v = Mat(-np.sum(self.target*np.log(aval) + (1-self.target)*np.log(1.-aval), axis=1, keepdims=True))
        v.creator = self
        self.deriv = (aval - self.target) / aval / (1.-aval)
        return v
